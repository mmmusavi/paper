...
@section('content_title')
    @if (!empty($volume))ویرایش  <span style="font-weight: 400;">{{$volume->name}}</span>@else افزودن مجله جدید @endif
@endsection
...
@section('content')
    ...
    {{ url('/dashboard/magazines/edit/'.$id) }} @else {{ url('/dashboard/magazines/new') }} @endif">
        {{ csrf_field() }}
